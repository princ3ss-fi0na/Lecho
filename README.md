...
body{
  ...
  color:#fff;min-height:100vh;display:flex;flex-direction:column;align-items:center;padding:16px;
...
.logo-card{
  width:100px;height:100px;border-radius:10px;background:linear-gradient(135deg,var(--accent-2),var(--accent));
  color:#111;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:60px;
  box-shadow:0 18px 40px rgba(0,0,0,0.55);flex-shrink:0;
}
.title{
  font-family:'Playfair Display', serif;
  font-weight:600;font-size:20px;letter-spacing:0.2px;color:var(--accent);margin:0;
  line-height:1;
}
.subtitle{font-size:12px;color:var(--muted);margin-top:4px;font-weight:500}

/* nav */
.nav-buttons{display:flex;gap:10px}
.big-btn{background:var(--accent);color:#111;padding:10px 14px;border-radius:var(--btn-radius);font-weight:800;min-width:110px;text-align:center;box-shadow:0 10px 24px rgba(0,0,0,0.28);cursor:pointer;border:none}
.balance{background:linear-gradient(90deg, rgba(255,255,255,0.06), rgba(255,255,255,0.03));padding:10px 14px;border-radius:12px;font-weight:800;color:var(--accent);display:flex;align-items:center;gap:8px;border:1px solid rgba(255,255,255,0.03);transition: all 0.3s ease;}
...
/* responsive: stack shelves on small */
@media(max-width:920px){ .shelves{grid-template-columns:1fr} }
@media(max-width:600px){ .shelves{grid-template-columns:1fr} .nav-buttons{display:none} }
...
.track-lane { display: flex; height: 60px; background: rgba(255,255,255,0.05); border-radius: 8px; position: relative; overflow: hidden; }
.horse { position: absolute; left: 0; top: 50%; transform: translateY(-50%); font-size: 32px; transition: left 0.1s linear; }
...
.finish-line { position: absolute; right: 0; top: 0; height: 100%; width: 4px; background: var(--accent-2); }
  ...
  <div id="screenHorseRace" class="screen">
    <div id="horseContent">
      ...
      <div id="raceTrack" class="race-track" style="display:none;">
        <div class="track-lane" data-lane="1"><div class="horse" data-horse="1">🐎</div><div class="finish-line"></div></div>
        <div class="track-lane" data-lane="2"><div class="horse" data-horse="2">🐎</div><div class="finish-line"></div></div>
        <div class="track-lane" data-lane="3"><div class="horse" data-horse="3">🐎</div><div class="finish-line"></div></div>
        <div class="track-lane" data-lane="4"><div class="horse" data-horse="4">🐎</div><div class="finish-line"></div></div>
        <div class="track-lane" data-lane="5"><div class="horse" data-horse="5">🐎</div><div class="finish-line"></div></div>
      </div>
...
$('saveNick').onclick = ()=>{ const n = $('nickProfile').value.trim() || genBotName(); sessionStorage.setItem('lecho_nick', n); showScreen('screenIntro'); };
$('saveStatus').onclick = () => {
...
$('confirmDiamondsToCoins').onclick = async () => {
  ...
  if (amt < 3 || amt % 3 !== 0) { alert('Количество должно быть кратно 3 и >=3'); exchangeProcessing = false; $('confirmDiamondsToCoins').disabled = false; return; }
  const coins = Math.floor(amt / 3);
  if (diamonds < amt) { alert('Недостаточно 💎'); exchangeProcessing = false; $('confirmDiamondsToCoins').disabled = false; return; }
  diamonds -= amt;
  balance += coins;
  saveBalance();
  alert(`Получено 🪙${coins}`);
...
$('confirmCoinsToDiamonds').onclick = async () => {
  ...
  if (amt < 1) { alert('Количество >=1'); exchangeProcessing = false; $('confirmCoinsToDiamonds').disabled = false; return; }
  const dias = amt * 2;
  if (balance < amt) { alert('Недостаточно 🪙'); exchangeProcessing = false; $('confirmCoinsToDiamonds').disabled = false; return; }
  balance -= amt;
  diamonds += dias;
  saveBalance();
  alert(`Получено 💎${dias}`);
...
function resetCasino() {
  ...
  if(won) {
    ...
    $('luckyBtn').onclick = () => { alert('Вы уже покорили лихорадку! Эта удача не повторяется'); };
  } else {
...
$('startRaceBtn').onclick = () => {
  ...
  if (bet < 2 || !selectedHorse || balance < bet) {
    alert('Недостаточно 🪙 или неверная ставка');
    return;
  }
  balance -= bet;
  saveBalance();
  ...
  let timeLeft = raceTime / 1000;
  $('#raceTimer').textContent = `Забег: ${Math.ceil(timeLeft)}с`;
  raceInterval = setInterval(() => {
    timeLeft -= 0.1;
    $('#raceTimer').textContent = `Забег: ${Math.ceil(timeLeft)}с`;
    if (timeLeft <= 3) {
      document.querySelectorAll('.horse').forEach(h => h.classList.add('boost'));
    }
  }, 100);
...
function startRace(duration, bet, selected) {
  ...
  const interval = setInterval(() => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (timeLeft <= 3) speed *= 1.5; // last 3s accelerate
      positions[i] += speed * (trackWidth / duration) * 100;
      horses[i].style.left = `${Math.min(positions[i], trackWidth)}px`;
...
function applyCasinoModifiers(isWin, winAmount, betAmount) {
  if (isWin && winAmount > betAmount && betDoublerActive) {
    ...
    alert('Удвоитель сработал!');
  } else if (!isWin && insuranceActive) {
    const refund = Math.floor(betAmount / 2);
    diamonds += refund;
    insuranceActive = false;
    alert(`Страховка вернула ${refund}💎`);
  }
  saveBalance();
}

$('startRoulette').onclick = () => {
  if (diamonds < 3) { alert('Недостаточно 💎'); return; }
  diamonds -= 3;
  saveBalance();
...
$('confirmColorChoice').onclick = () => {
  if (!selectedColor || diamonds < 2) { alert('Выберите цвет или недостаточно 💎'); return; }
  diamonds -= 2;
  saveBalance();
  const colors = ['🔴', '⚫️', '🟢'];
...
function goldRushBet() {
  if (balance < 5) { alert('Недостаточно 🪙'); return; }
  balance -= 5;
  saveBalance();
...
const symbols = ['🍒', '🍋', '🍊', '🔔', '💀'];
$('leverBtn').onclick = () => {
  if (diamonds < 10) { alert('Недостаточно 💎'); return; }
  diamonds -= 10;
  saveBalance();
  const reel1 = $('reel1');
  const reel2 = $('reel2');
...
ui.returnBtn.onclick = ()=>{
  ...
  if(useVoucherIfAvailable()){
    ...
    if(balance < RETURN_COST){ alert(`Нужно ${RETURN_COST} 🪙 чтобы вернуть карту отправителю.`); return; }
...
ui.lechoBtn.onclick = ()=>{
  if(processing) return;
  const p = players[0];
  if(p.antiLecho) { alert('Ваше объявление Лечо заблокировано!'); return; }
  if(p.finished) { alert('Комбинация собрана - ожидайте конца игры'); return; }
  ...
  } else { alert('Вы не собрали комбинацию!'); log('Попытка объявить Лечо без комбинации.'); }
};
...
function buyItem(id){
  ...
  if(balance < item.price){ alert('Недостаточно 🪙'); return; }
...
function useItemEffect(type, isFreeSample = false) {
  if (type === 'hint') {
    ...
    if (possibleCombos.length > 0) {
      alert(`Возможные комбинации: ${possibleCombos.join('; ')}`);
    } else {
      alert('Нет возможных комбинаций');
    }
  } else if (type === 'peek') {
    if (deck.length > 0) {
      const top = deck[deck.length - 1];
      alert(`Верх колоды: ${top.rank}${top.suit}`);
    } else {
      alert('Колода пуста');
    }
  } else if (type === 'skip') {
    ...
    if (aliveBots.length > 0) {
      ...
      alert(`${randomBot.name} имеет: ${botCombo || 'нет комбо'}`);
    } else {
      alert('Нет ботов для разоблачения');
    }
    ...
    if (aliveBots.length > 0) {
      ...
      if (randomBot.hand.length > 0) {
        ...
        alert(`У ${randomBot.name}: ${randomCard.rank}${randomCard.suit}`);
      }
    } else {
      alert('Нет противников');
    }
    ...
    alert('Удвоитель активирован для следующей победы в казино');
  } else if (type === 'insurance') {
    insuranceActive = true;
    alert('Страховка активирована для следующего проигрыша в казино');
